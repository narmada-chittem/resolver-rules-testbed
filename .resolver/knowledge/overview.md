...
Per `README.md`, this is a seeded testbed repo ("resolver-rules-testbed") for a resolver+ingestion manual test matrix. The source is one file, `src/greeter.ts`, exporting three functions: `capitalizeName(name)`, `greet(name)` (delegates to `capitalizeName`), and `farewell(name)`. A test file, `src/greeter.test.ts`, now exists using Node's built-in `node:test`/`node:assert/strict` — there is still no `package.json`, build config, or test-runner config, so tests must be run directly (e.g. `node --test src/greeter.test.ts`). There is no `src/ui/` directory — despite guidance files referencing pnpm, component conventions, and UI-layer rules.
